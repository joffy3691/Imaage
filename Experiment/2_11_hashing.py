...
my_img.srsa_key_position = {}

for i in range(256):
    rsa_key_position[rsa_keys[i]] = iave(filename,  exif=exif_dat)
...
exif = {
    TAGS[k]: v
    for k, v in my_img._getexif().items()
    if k in TAGS
}

rsa_keys1 = exif.get('MakerNote').decode("utf-8","ignore").split()
rsa_key_position1 = {}

for i in range(256):
    rsa_key_position1[rsa_keys1[i]] = i
