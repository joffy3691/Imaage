...
import base64
import time
import encrypting_copy
import decrypting_copy
...
while True:
    ...
    if event == 'Encrypt' and values['key']:
        ...
        encrypting_copy.encryption(values['-FILE LIST-'][0], values['key'])
    ...
    if event == 'Decrypt' and values['key']:
        ...
        tic = time.perf_counter()
        decrypting_copy.decryption(values['-FILE LIST-'][0],values['key'], values['Rsakey'], values['Publickey'])
        toc = time.perf_counter()
